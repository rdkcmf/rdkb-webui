...
if (($_SESSION["loginuser"] == "mso") && ($i == 5 || $i == 6)) {
	//var jsConfig = '{"radio_restore":"true", "ssid_number":"5"}';
	if("true" == $arConfig['radio_restore']){
	...
	// check if the LowerLayers radio is enabled. if disable, no need to configure following
		...
		if ("true" == $arConfig['radio_enable'] || "true" == $arConfig['radio_reset'])
		{
			...
			if($validation) $validation = isValInRange($arConfig['max_client'], 1, 127);
			if($validation) $validation = (preg_match("/^[1-9]\d*$|^[0-9]\d\d*$/", $arConfig['DSCPMarkPolicy'])==1);
			if($validation) $validation = (preg_match("/^[1-9]\d*$|^[0-9]\d\d*$/", $arConfig['KeepAliveCount'])==1);
			if($validation) $validation = (preg_match("/^[1-9]\d*$|^[0-9]\d\d*$/", $arConfig['KeepAliveInterval'])==1);
			if($validation) $validation = (preg_match("/^[1-9]\d*$|^[0-9]\d\d*$/", $arConfig['KeepAliveThreshold'])==1);
			if($validation) $validation = (preg_match("/^[1-9]\d*$|^[0-9]\d\d*$/", $arConfig['KeepAliveFailInterval'])==1);
			if($validation) $validation = (preg_match("/^[1-9]\d*$|^[0-9]\d\d*$/", $arConfig['ReconnectPrimary'])==1);
			if($validation){
