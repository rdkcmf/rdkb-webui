...
$(document).ready(function() {
	...
	}, 'Choose a different Network Name (SSID) (<?php echo $wifi_value["freq_band"]; ?>) than the one provided on your gateway.');
	...
	}, 'Choose a different Network Password (<?php echo $wifi_value["freq_band"]; ?>) than the one provided on your gateway.');
	...
	}, 'Choose a different Network Name (SSID) (<?php echo $wifi_value["freq_band1"]; ?>) than the one provided on your gateway.');
    $.validator.addMethod("not_defaulPassword2", function(value, element, param) {
		//prevent users to set defaul-Password as Password
		return value != "<?php echo $defaultKeyPassphrase2; ?>";
	}, 'Choose a different Network Password (<?php echo $wifi_value["freq_band1"]; ?>) than the one provided on your gateway.');
...
<div id="content">
	...
	<div class="module forms">
		...
		?>
			...
			<div class="form-row odd">
				<label for="network_name">Wi-Fi Network Name (<?php echo $wifi_value["freq_band"]; ?>):</label>
				<input type="text" size="23" value="<?php echo htmlspecialchars($network_name);?>" id="network_name" name="network_name" class="text" />
			</div>
			<div class="form-row">
				<label for="security">Encryption Method (<?php echo $wifi_value["freq_band"]; ?>):</label>
			...
			<div id="netPassword">
				<div class="form-row odd">
					<label for="network_password">Network Password (<?php echo $wifi_value["freq_band"]; ?>):</label>
			...
			<div class="form-row odd">
				<label for="network_name1">Wi-Fi Network Name (<?php echo $wifi_value["freq_band1"]; ?>):</label>
				<input type="text" size="23" value="<?php echo htmlspecialchars($network_name1);?>" id="network_name1" name="network_name1" class="text" />
			</div>	
			<div class="form-row">
				<label for="security1">Encryption Method (<?php echo $wifi_value["freq_band1"]; ?>):</label>
				<select name="encryption_method1" id="security1">
					...
					<option value="WPA2_PSK_AES" 		title="WPA requires an 8-63 ASCII character password or a 64 hex character password. Hex means only the following characters can be used: ABCDEF0123456789." <?php if ("WPA2_PSK_AES"==$security1)        echo "selected";?> >WPA2-PSK (AES)(Recommended)</option>
					...
					<option value="WPAWPA2_PSK_TKIPAES" 	title="WPA requires an 8-63 ASCII character password or a 64 hex character password. Hex means only the following characters can be used: ABCDEF0123456789." <?php if ("WPAWPA2_PSK_TKIPAES"==$security1) echo "selected";?> >WPAWPA2-PSK (TKIP/AES)</option>
				</select>
			</div>
			<div id="netPassword1">
				<div class="form-row odd">
					<label for="network_password1">Network Password (<?php echo $wifi_value["freq_band1"]; ?>):</label>
